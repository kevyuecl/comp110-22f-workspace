...
prompt_rand_num_letter_guess: str = input(f"What is your {secret_word_len} letter guess? ")
# Prompt word guess from the user
while (len(prompt_rand_num_letter_guess) != secret_word_len):
    prompt_rand_num_letter_guess = input(f"That was not {secret_word_len} letters! Try Again: ")
...
guess_emoji: str = ""
while (word_guess_index < secret_word_len):
    ...
    # If user guess index is the same as the secret word the index, add a green box. Otherwise, declare and initialize boolean variable regarding whether a character is present or not to false and also declare and initialize an alternate word index to 0. 
        ...
        while (bool_guessed_character is not True and (alt_word_guess_index < secret_word_len)):
            ...
            # If the alternate index of the secret word is the index of the user guess, the guess character is in the word. Otherwise, increment alternate index variable. 

        if (bool_guessed_character is True):
